...
-   <div>
    ...
    examen_groupe_A = examen$note[examen$groupe == "A"]
    examen_groupe_C = examen$note[examen$groupe == "C"]

    boxplot(examen_groupe_A, examen_groupe_C, names = c("Classe A", "Classe C"))
    rug(examen_groupe_A, side=2)
    rug(examen_groupe_C, side=4)
...
> En se basant uniquement sur les boxplots, on peut observer que le groupe A semble avoir des notes plus élevées que le groupe C, car la médiane du groupe A est plus haute et la boîte (interquartile range) se situe globalement à des valeurs plus élevées.
